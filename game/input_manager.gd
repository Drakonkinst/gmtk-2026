...
func _to_drawing_pos(screen_pos: Vector2) -> Vector2i:
    var drawing_pos_x: int = round(screen_pos.x - DRAWING_OFFSET_X)
    var drawing_pos_y: int = round(screen_pos.y - DRAWING_OFFSET_Y)
    if drawing_pos_x >= Drawing.WIDTH * SCALE_FACTOR || drawing_pos_y >= Drawing.HEIGHT * SCALE_FACTOR || drawing_pos_x < 0 || drawing_pos_y < 0:
